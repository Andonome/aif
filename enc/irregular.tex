...
{Easy Job}% NAME
{The troupe must hunt down a dwarf accused of failure to pay taxes to the \glsfmttext{templeOfBeasts}}% SUMMARY

\begin{boxtext}
  On this \showTemperature\ afternoon, a two-horse cart shuffles along the distant road, towards your \gls{broch}.
  The driver waves silently, beckoning you to walk down hill and speak.
\end{boxtext}

Trader \marketFence\ tugs the stress from his long, black beard, and explains that the \gls{village} up the road has smelled \pgls{basilisk} last night, and need \gls{guard} urgently.

\Gls{jotter}~Filchvore refuses to let the \glspl{pc} leave until \marketFence\ drops the dwarf-bomb:

\begin{quotation}
    That's fine then.
    I guess the fancy-axe dwarf can take the beast.
    He said he was a monster-hunter.
\end{quotation}

At this, \Gls{jotter}~Filchvore goes red with fury, shouting that any sale on \glspl{basilisk} hides, \glspl{crawler} webs, or the like, must be approved by the \gls{templeOfBeasts}.

\begin{quotation}
    \ifnum\thetemperature>0
      Get out there and find out if that dwarf has been selling hides without giving the temple its due.
      And kill that \gls{basilisk} while you're at it.
    \else
      If \pgls{basilisk} woke from hibernation, it'll be dead by now.
      But if that dwarf sold the body, the stench will still be on his beard.
    \fi

    The \gls{village} is fifteen miles away but
    there's \pgls{bothy} half way.
    March fast and you can sleep there then see the \gls{village} tomorrow morning.
\end{quotation}

\noindent
The \gls{village} in question is fifteen miles down the road, so the \glspl{pc} should leave at dawn.
Find the \gls{village}'s name and details in a market handout.
...
(noise or light has a $\frac{1}{\dicef{6}}$ chance of summoning \pgls{monster}).
...
It can curdle yesterday's dinner, and inflicts \pgls{ep} on each character.
Nearby animals begin to panic.
The troupe will not sleep well, so they must take another \gls{ep} due to the dreamless night.

\segment{Forest}% AREA
{Wyrmsign}% NAME
{Patches of torn-up earth suggest something massive searches the earth}% SUMMARY

Massive claws have torn through the earth
\ifnum\thetemperature=0
  and snow.
  The tracks are easy to follow, but show no feet --- just a slithering body scraping through the snow.
\else
...
\ifnum\thetemperature=0
  Patches of melted snow show barren earth with small singe-marks on the dead grasses underneaht.
\else
...
results in a single, flaming, belch (dodge at \tn, or take \dmg{-1}).
...
\begin{boxtext}
  \ifnum\thetemperature=0
    A little frozen river leads into a patch of willow trees,
    then into the open snow.
    In the centre sits
  \else
    A small river trickles through a patch of willow trees,
    then into the open, and down
  \fi
  a hole the size of a portcullis.
\end{boxtext}
