...
\person{1}% STRENGTH
	...
	{0}% DR
	{1}% COMBAT
	{Beast Ken 2, Empathy 1, Survival 2}% SKILLS
...
\Gls{traitor} has not informed the rest of \gls{nightguard}, such as \gls{captain} of his activities, so if he suspects the characters are acquainted with \gls{nightguard}, he will remain hidden, or start spinning lies about how the place is top secret business of \gls{king}.
...
If news of their whereabouts is brought to \gls{nightguard},\footnote{See page \pageref{guardstation} for more on the guard station.}
...
{A member of \glsentrytext{nightguard} sent to track down where the dead have come from has returned as a ghoul}% SUMMARY
...
It was built over a mana lake, so the entire town will allow any spellcasters to regenerate mana quickly.

\humandiplomat[\npc{\M}{Guildmaster Solus}]
\label{solus}

If you use this area for a ghoul attack with \gls{necromancer}, the ghouls approach underwater and come up from the river.
