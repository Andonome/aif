...
\newcommand\politeSpell{
  \spell{Polite Request}% Name
    {Detailed, Duplicated}% Enhancements
    {Warp}% Action
    {Fate, Water}% Spheres
    {\roll{Wits}{Academics}}% Resist with
    {The caster praises the targets' excellent manners, and they become enamoured with their own politeness.
    Any deviation from proper etiquette inflicts a \arabic{spellPlusOne}~Penalty to all Mind \glspl{attribute} for \pgls{interval}, and each breach adds another}% Description
    {Breaches of etiquette include swearing, defending oneself without formally accepting the `duel', suggestions of bigotry, correcting others, and expressions of disappointment of any kind.

    The \gls{spell} does not pick up on sarcasm.}
}


\newcommand{\enchantedGoblin}[1][\npc{\T[\arabic{r4t6}]\N}{Peace-Keeping Goblins}]{
  \Person{#1}%
    {{r3}{r2b}{rn1t2}}% BODY
...
\newcommand\feyCommentaryLocations[1][t]{
  \playCommentary[#1]{
    \begin{description}
    \item[\gls{gm}:]
...
\newcommand\feyCommentaryPlan[1][t]{
  \playCommentary[#1]{
    \begin{description}
      \item[Player 2:]
      (Mildrain)
      ...
      They say the cure is to take an oath at \gls{oathtower} (after they arrest you, in \gls{oathtower}), because taking an oath always gives you a clear sense of purpose.
      \item[Player 2:]
      (Mildrain)
      Okay, `thanks'?
      But what's the plan?
      ...
      (Cleftbarb)
      I'll speak quietly.
      {\small We need Earth \glspl{ingredient}, so we're going to steal them from \gls{oathtower}.
