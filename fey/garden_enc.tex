...
\segment{plateauGardens}% AREA
{Disturbing the Peace}% NAME
{The troupe find out how strange they seem when they light a fire}% SUMMARY

When night falls, most local elves simply lie down and sleep on the ground.
They don't suffer from rain, heat, or cold.%
\exRef{stories}{Stories}{elvenInheritance}
If the \glspl{pc} light a fire, the local elves will arrive to give polite
hints about the dangers of lighting a fire and attracting nearby
\glspl{monster}.

\begin{quotation}
  I saw your fire from three plateaus over, and had to ask what you were doing.
  It seems a very \emph{brave} hour to attract local, wandering,
  \glspl{monster}, since one cannot see so well in the dark.

  Don't worry about us, we can vacate the area, so as not to interfere with
  whatever feels attracted to the fire.
\end{quotation}

This \gls{segment} exists to highlight how strange the \glspl{pc} seem to the
elves, and how they upset the local order.
...
If the \gls{woodspy} attacks \pgls{npc}, you can resolve the fight by just
comparing each \gls{npc}'s \gls{cr} (highest wins), instead of rolling dice
alone.%
