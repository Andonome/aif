...
\enchantedHobgoblin[\npc{\T[4]\N}{Hobgoblins Grawlix \& Sillabery}]

\enchantedHobgoblin[\npc{\T[3]\N}{Hobgoblins}]

\enchantedGoblin[\npc{\T[4]\N}{Hobgoblins Glyph \& Majiscule.}]
\label{Majiscule}
...
\begin{itemize}
  ...
  I don't want to hear any more tuneless songs from the \glspl{consumer}.
  \item\it
...
they discuss the grand spell, and request the \glspl{pc} help them in \gls{harvesting} the necessary \glspl{ingredient}.
...
\gls{juliet} hands them a flower or two, and just says `\textit{flower of enlightenment, for the weight of the world}' (she doesn't speak \gls{tradeTongue} well, but anyone who speaks Elvish can clarify).
...
\begin{speechtext}
  My father has played a dangerous game with the \glspl{consumer}.
  ...
  If the \glspl{consumer} suspected that \gls{MindElder} wants to reduce them, they would begin to push back against their oaths, or simply leave the area, past where he can reach them.

  We do not want that.
  We do not want to abandoned the \glspl{consumer} to their instincts.
\end{speechtext}
...
and engineer \pgls{flood}, perhaps by creating or destroying a damn.%
...
Whatever their plan, it should be abstracted to \pgls{natural}; and if the roll fails, they can either change their tools to provide bigger Bonuses or change their methods and make a new roll with a completely different plan.
...
\begin{boxtext}
  A little band of goblins quick-march through \gls{ravencops}, with constantly searching eyes.
  They approach, then ignore you.
  One looks familiar, despite hiding his face.
\end{boxtext}

\paragraph{If the \glspl{pc} ask about their mission politely,}
the familiar goblin (Majiscule, from \vpageref{Majiscule}) explains frankly that \Gls{MindElder} sent them to spy on \gls{romeo}.
The others continue marching.

\enchantedGoblin

\paragraph{When the goblins find \gls{romeo}}
they split up, and each make solitary, voyeuristic, observations of his journey to \gls{sunway}, until he speaks with \gls{juliet}.
Then one-by-one, the goblins return to report to \gls{MindElder}, who guesses the couple's intentions from the descriptions of the songs and symbols%
\footnote{As noted in \gls{segment}~\vref{sunwayGlyphs}.}
on the trees.

\Gls{MindElder} sends goblins to wake \pgls{sepulchre} of \glspl{ogre} with a meagre breakfast, and pass along the order to run at full pace towards the \gls{sunway}, and stop the ritual by screaching, tearing down trees, and `confiscating' `items of ritual abuse' (\glspl{boon}).

\act{spot a goblin}{Speed}{Vigilance}{\roll{Dexterity}{Stealth}}
But the \gls{tn} only helps find one goblin, since the band are using \pgls{bandAct}.
On failure, the \glspl{ogre} within \pgls{interval}, and if the troupe don't half their advance, the grand \gls{spell} will lie in ruins, with the \glspl{boon} scattered for nothing.

Space will be on the troupe's side; anyone can see these \glspl{ogre} coming from two miles away, despite the dense \gls{ravencops} forest.
...
\begin{boxtext}
  ...
  Two \glspl{consumer} stand on the cooker holding your memory of learning the Air \gls{sphere}.

    ``\textit{Sorry, wrong person!}'', the little \gls{consumer} says in Elvish.
