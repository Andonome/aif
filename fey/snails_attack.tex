\thread[coppernut]{Slowburn Snails}

\widePic{shadows/snail_attack}

\segment{coppernut}% AREA
...
\begin{boxtext}[Do you get involved in the debate?]
...
they can arrive on time with a \roll{Speed}{Athletics} roll at \tn[5].
...
\iftoggle{intro}{
  \ifnum\value{Speed}=-3
    That `\gls{ap}~$\frac{1}{2}$' means the snail acts on the first \gls{round}, then loses \pgls{round}, and so on.
  \fi

  \paragraph{If the \glspl{pc} try to trick the snail}
  it works, because snails are stupid.
}{}

\paragraph{The foot on top}
belongs to \gls{SnailTamer}, who took a nap while riding the snail.
If he wakes, he'll try to control the snail, but it takes him a while.
...
he says he has a letter from \gls{MindElder}, and must deliver it at once.
...
\begin{exampletext}
  ...
  Eventually, everyone abandoned their area, and fled to other \glspl{village} where they had family, or had to enter \gls{ftown} to beg for food.
\end{exampletext}

\Gls{coppernut} lies empty now, except for \pgls{crawler}, who
\ifnum\thetemperature=0%
  hibernates in a house's bedroom.
\else%
