...
\playCommentary{
  \begin{description}
    \item[\gls{gm}:]
    \Pgls{broch} is a tower with thick, stone walls, and a couple of narrow entrances.
    \item[Player 1:]
    (Sinkmaul)
    Sounds charming.
    \item[\gls{gm}:]
    Yea, and it grows on you even more when long legs climb the tower, trying to find a way in.
    That's a common event here, because the tower makes a lot of noise to draw in all the predators in the forest.
    \Pgls{soldier} wanders past, going upstairs with his pipes to attract all the creatures he can.
    \item[Player 1:]
    (Sinkmaul)
    He doesn't sound very bright.
    \item[Player 2:]
    (Mildrain)
    Yea, staying quiet sounds like a better idea.
    Proper idiot\ldots
    \item[\gls{gm}:]
    Right, starting over\ldots
  \end{description}
}{
  Beginnings are always tricky.
  I have never gotten the hang of them, but writing a bullet-point list always helps cement knowledge into intention.
  
  Another good standard is introducing players slowly.
  Handing out random characters is great, because it lets you start immediately.
  But rolling random characters together can help introduce the world better, since everyone focusses on each background for a moment, and they all understand they're now \glspl{fodder} in the \gls{guard}.
}
