...
Once \pgls{pc} drops down, the goblins start throwing rocks at them; the player roll's their characters \roll{Speed}{Vigilance} (see the goblin \glspl{statblock} for the \gls{tn} and Damage).
A successful roll means the \gls{pc} can dodge the rocks by \gls{running}.
A failed roll means Damage, and a tie means the player dodges without \gls{running} anywhere, and any \gls{pc} behind them may be hit by a rock.
