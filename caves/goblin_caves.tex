...
\wayOut{Past the bridge, a slender crack leads to the}{wormTunnel}
...
\wayOut{The only ways out are back, or down to the river in front of the}{waterMaze}
...
requires a \roll{Speed}{Caving} roll at \tn[9].
...
\wayOut{The troupe will exit through a narrow crack, at the}{umberHulk}
...
\wayOut{Below, the \glspl{pc} find new problems, just before reaching the}{smokePassage}
...
\wayOut{The only way out leads to the}{smokePassage}
After \pgls{interval}, goblins arrive below to set a fire at the \stateArea{darkTunnel}'s exit.
This will irritate the umber hulk, causing it to chase the goblins, who flee across the river in the \stateArea{blindFish}.
...
\wayOut{This lower passage leads to the}{skeinSwarm}

\wayOut{If the troupe run up the little ledge the goblins stand on, it leads to a cavern full of large rocks, then on to the}{mushroomsTunnel}
...
\wayOut{A deeper tunnel leads down through miles of darkness, until it hits the}{darknessAndFire}.
...
It also presents a serious problem for the \glspl{pc}' mission, with very little solution.
...
\subsection{Back at the \Glsfmttext{broch}}
