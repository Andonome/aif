...
A satchel still contains two bees-wax candles which can burn for an hour each, a tinder-box, \lootMedium, and slightly-expired \rations.
...
\smolMapPic{cave_3}{
  \ref{firstBridge}/50/75,
  \ref{gentlePassage}/92/15,
}
...
The second warning sign is a dead gnome.
...
A coin purse also contains \lootMedium.
...
Each morning and evening, the local \gls{broch} sounds their pipes, which will let the \glspl{pc} know where to go.
At any other time of day, the troupe can hear a nearby river, which will eventually lead to \pgls{village}, not far from Stoatfen~\Gls{bothy}.

\ifcase\value{temperature}
  A vicious breeze blows, and in the distance, wolves howl.
  ...
  Meanwhile, \pgls{crawler} watches them from above (they spot it immediately).
  If they retreat, it leaves them.
  But if they remain visible, it attacks.
