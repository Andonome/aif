...
\newcommand\lostGuard{
  \humanthief[\NPC{\glsentrysymbol{sylf}\M\Hu}{\Glsfmttext{fodder}~\scaredFodder}%
    {mid-forties, filthy face, and flustered}% DESCRIPTION
    {grimaced-smile}% MANNERISM
    {to accept reality}]% WANTS
}
...
\newcommand\playCommentaryTorches[1][t]{
  \playCommentary[#1]{
    \begin{description}
      \item[\Gls{gm}:]
      ...
      \arabic{r6}~\glspl{fp}.
      \item[Player 1:]
      That must be nearly night, and
      I don't have any food left, or \glspl{torch}.
      \item[Player 2:]
      I have two \glspl{torch}, but we should save them for later, and hurry up.
      \item[\Gls{gm}:]
      If you're in the dark, how will you find your way?
      \item[Player 1:]
      Use the sword-scabbard as a walking stick?
      \item[\Gls{gm}:]
      Okay, but everyone will still slow down to feel each footstep carefully, and travel just two miles over the \gls{interval}.
      \item[Player 2:]
      Can't we rush?
      \item[\Gls{gm}:]
      In the darkness?
    ...
    The \gls{gm} already knows the rules for \gls{blackWalking}, but asking the players how they walk pulls the table into a single, clear, notion of what's happening.

    The next \gls{interval} begins the night, so the troupe will have to find somewhere to lay down and sleep for the \gls{interval}, or suffer 3~\glspl{ep}.
    And staying awake throughout \pgls{vigil} in the darkness won't be easy.
  }
}
