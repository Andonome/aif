...
Veklash, \gls{notary} at the \gls{weaversGuild}, is heading North with a few other traders.
He has an appointment to take valuables from \pgls{village}'s \gls{warden} for safe keeping, as everyone fears goblins taking their valuables.

On the road, he tells the characters his tragic story.
...
\begin{speechtext}
  ...
  Lost my life savings, but at least left alive, unlike the others.
\end{speechtext}

\humantrader[\NPC{\glsentrysymbol{abderian}\M\Hu}{Veklash}%
  {wide hat, kind smile}% DESCRIPTION
  {shakes head mournfully}% MANNERISM
  {to tell his tale of woe}% WANTS
  \npcQuote{Travel straight, never curious}]% QUOTE

\begin{speechtext}
  Here we are at last -- Stoatfen \Gls{bothy}.
...
\begin{itemize}
  ...
  \ifnum\value{temperature}<2
    \item
    Check the \gls{bothy} has firewood (it does not).
  \fi
\end{itemize}

\paragraph{Once the \glspl{pc} check the \gls{bothy},}
\ifnum\value{temperature}<2
  they'll have to collect firewood while the traders tend to their horses.
\else
  everyone settles in and tends to their horses.
\fi
\iftoggle{verbose}{
...
{The \glsfmttext{guard} \glsfmttext{fodder} fled from the goblin caves, and wants to know if he can return}% SUMMARY
...
\begin{exampletext}
  ...
  Four days ago, \pgls{susjot} ordered him and all the new \glspl{fodder} to enter an abandoned mine, kill the goblins, and return with the treasure they stole from Veklash and the rest of that caravan.
...
\begin{boxtext}
  ...
  \begin{speechtext}
    ...
    \Pgls{susjot} sent us to the old mine, to kill the goblins and return with the treasure they stole.
...
\begin{exampletext}
  ...
  \Gls{susjot} has figured they saw the chest of \glsentrylongpl{gp} and turned to banditry.
\end{exampletext}
...
\Pgls{susjot} alights, and asks about the logs.
...
\begin{speechtext}
  ...
  The last lot of \glspl{fodder} who went up stole some \glsentrylongpl{gp}, and didn't even kill the goblins.
