...
\begin{itemize}
  ...
  A tie indicates the character has lost their backpack, or other item.
...
A basilisk can fire its acidic breath forward up to ten steps away, inflicting $1D6-2$ acidic Damage.
...
\begin{itemize}
  ...
  On a tie, they have a moment to prepare before the bear wakes.
  \item
...
\begin{itemize}
  ...
  On a tie, they notice while already descending the muddy slope.
  The chitincrawler then immediately attacks from the top.
...
\begin{itemize}
  ...
  The parent has left, so a Speed + Athletics roll allows a party member to snatch the eggs before a parent returns (TN 9).
  \item
...
\begin{itemize}
  ...
  \ifodd\value{temperature}=1
    ...
    Success means they have spotted a woodspy upstream, plopping into the water in order to lay an ambush, while hitting the \gls{tn} precisely means they have only one round to prepare.
...
Some ooze also have the Projectiles Skill, indicating they can shoot acid up to 5 steps away.
...
\begin{itemize}
  ...
  Have the party roll Wits + Vigilance, TN 9 due to the twilight, to notice it in the distance.
  \item
...
Once caught in the web, a resisted Strength roll against the web is required (costing 1 \gls{ap} -- each spider's web has a Strength rating equal to its own plus one.
A tie indicates that they must spend 4 \glspl{ap} to resist the webbing.
