...
Just about every \gls{temple} in the city holds something down there, and none of them trust the others.
...
\begin{speechtext}
  We're not lighting the fire to invite creatures up.
  You listening to me?
...
\begin{boxtable}
  ...
  \Gls{tn}~9 & They break the lock-mechanism while slamming the door open, which breaks a mirror and some half-prepared Fate~\glspl{ingredient} (-\arabic{r6}~\glspl{fp}). \\
\end{boxtable}
...
To one side sits a barrel of 12~\glspl{torch}, and a barrel with an acidic ooze.%
\footnote{The cheese-cloth lining stops it escaping.}
Neither are labelled.
...
They do not believe they are responsible for maintenance, so they have not fixed the door, which is currently hanging on by a single hinge.
Any vigorous pushing against the door will make it topple.

Attacking the frozen \glspl{monster} poses no challenge; the \glspl{pc} can
simply roll Damage with a +2~Bonus and an automatic \gls{vitalShot}.  However,
the \glspl{monster} are waking up, so every strike has a $\frac{1}{6}$ chance
of waking the nearest.

In total, the chamber contains:

\umberhulk[\npc{\T[2]\A}{\Glspl{hulk} `3' \& `5'}]

\Glspl{guard} paint numbers on the \glspl{hulk}' shells to keep track of which is which, and how long each has been in the fridge.
\Gls{crawler} hairy bodies are not as easy to paint, so the \glspl{guard} don't.

\chitincrawler

\umberhulk[\npc{\T[2]\A}{\Glspl{hulk} `4' \& `6'}]

\basilisk[\npc{\A}{\Glsfmttext{basilisk}}]

\chitincrawler

\begin{boxtext}
  The back wall looks like a mound of frozen toothpicks.
\end{boxtext}

\dragon[\NPC{\E\glsentrysymbol{fiend}}{Syadz}%
  {one eye, slinky, yellow, writhing spine and lazy feet}% DESCRIPTION
  {defensive tail-whips}% MANNERISM
  {to establish a safe escape}% WANTS
]
...
This place is littered with every type of fabric imaginable, a few barrels of wine, and a few more snowmen dotted around the walls (mostly clothed, others anatomically exaggerated).
...
Banned books and pamphlets remain here, where uneducated people won't be in danger of getting ideas.
...
Behind the locked door, the cupboard has several shelves, sagging from weight.
