...
\iftoggle{core}{}{\input{fenintro.tex}}
...
It has a magical portal to Eastlake, not far from the \gls{college}; it has another to Quennome, where the elves refuse to acknowledge his rule.
...
The \gls{guard} and those traders who can afford the privilege use the \gls{shatteredcastle} to travel across Fenestra faster than anyone else possibly could.
...
Many priests claim that they should govern the \gls{alchemists} by divine right, but so far \gls{king} has not entertained these notions.
...
Her journeymen travel with the \gls{guard} routinely, and regularly pretend to be one of them to check on whether small nobles have created any weapon stocks beyond what the law allows.
...
\begin{enumerate}
  ...
  \item
  Obey all commands from the \gls{alchemists}.
...
\widePic{Tom_Prante/ancient_valley}
